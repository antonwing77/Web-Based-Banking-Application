...
#connects to database BankApp with password !12345
conn = psycopg2.connect(dbname = 'BankApp', user = 'postgres', password = '!12345', host = 'localhost', port = '5432')
...
def create_role(role1):
    conn1 = psycopg2.connect(dbname='BankApp', user='postgres', password='!12345', host='localhost', port='5432')
    cur1 = conn1.cursor()
    try:
        cur1.execute(f"CREATE ROLE {role1}")
        print("Created role " + role1)
        conn1.commit()
    except psycopg2.errors.DuplicateObject:
        print("Role " + role1 + " already exists")
        conn1.rollback()
    finally:
         cur1.close()
         conn1.close()
if __name__ == '__main__':
    # parallelizes role creation
    with Pool(processes=3) as pool:
        pool.map(create_role, roles)
